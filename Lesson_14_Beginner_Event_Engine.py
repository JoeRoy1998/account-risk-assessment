...
def run_detection_engine(account, severity_rules):
    ...
    if role in ROLE_RULES:
        ...
        if current_index < len(SEVERITY_ORDER) - 1:
            final_severity = SEVERITY_ORDER[current_index + 1]
    
    if baseline_sev != final_severity:
        reason = f"{failed_logins} failed logins meets {baseline_sev} threshold ({current_threshold}). Escalated due to Executive role."
    else: 
